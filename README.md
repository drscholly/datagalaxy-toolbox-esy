...
```
datagalaxy-toolbox.exe export-glossary [-h] --url URL --token TOKEN --workspace WORKSPACE [--version VERSION] [--bulktree_mode]
```
...
```
datagalaxy-toolbox.exe export-dictionary [-h] --url URL --token TOKEN --workspace WORKSPACE [--version VERSION] [--bulktree_mode]
```
 `--version` is only for a versioned workspace.

 `--bulktree_mode` option will export objects in bulktree mode, ready to be imported (if you have more than 5000 objects, you will get multiple bulktrees to handle volume). 

#### export-dataprocessings

```
datagalaxy-toolbox.exe export-dataprocessings [-h] --url URL --token TOKEN --workspace WORKSPACE [--version VERSION] [--bulktree_mode]
```
 `--version` is only for a versioned workspace.
...
```
datagalaxy-toolbox.exe export-usages [-h] --url URL --token TOKEN --workspace WORKSPACE [--version VERSION] [--bulktree_mode]
```
 `--version` is only for a versioned workspace.
